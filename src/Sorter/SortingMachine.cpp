...
namespace Sorter {
    ...
    SortingMachine::initialise() {
        std::vector<Bin> bins = std::move(File::read_array<Bin>(__settings->bins_path, __settings->bins_file_type));
        std::vector<Pattern> patterns = std::move(File::read_array<Pattern>(__settings->patterns_path, __settings->patterns_file_type));
        for (Bin& bin: bins) {
