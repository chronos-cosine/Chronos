...
namespace Sorter {
    ...
    Sorter::process_job(Job& job) {
        ...
        keyword_boolean_matcher.match_boolean(__match_patterns[std::shared_ptr<Job>(&job)], __match_bins[std::shared_ptr<Job>(&job)]); 
        
        notification << "Checking tree parents " << job.get_filename();
        notify(notification);
        
        bin_parent_matcher.match_parents(__match_bins[std::shared_ptr<Job>(&job)]);
        
        notification << "Writing the results for " << job.get_filename();
        notify(notification);
        
        __result_file_writer.write(job, __match_patterns[std::shared_ptr<Job>(&job)],
                __match_bins[std::shared_ptr<Job>(&job)], 
